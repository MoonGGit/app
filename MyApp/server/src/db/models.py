...
class Image_Path(Base):
    ...
    fk_user_other_id = Column(String(30), ForeignKey(
        'user_other.id', ondelete='CASCADE'), nullable=True)
    fk_user_other_oauth = Column(String(10), ForeignKey(
        'user_other.oauth'), nullable=True)
    no = Column(Integer, autoincrement=True, nullable=False)
    path = Column(String(100), nullable=False)
    user_paths = relation('User', backref=backref('user_paths'))
    # user_other_paths = relation(
    #     'User_Other', foreign_keys='[Image_Path.fk_user_other_id, Image_Path.fk_user_other_oauth]', backref=backref('user_other_paths'))

    __table_args__ = (
        CheckConstraint(r'''fk_user_id is not null and fk_user_other_id is null and fk_user_other_oauth is null or
        fk_user_id is null and fk_user_other_id is not null and fk_user_other_oauth is not null'''),
    )

    __mapper_args__ = {'primary_key': [fk_user_id]}

    def __init__(self, fk_user_id, fk_user_other_id, fk_user_other_oauth,  path):
        self.fk_user_id = fk_user_id
